...
def get_bert_tokenizer(
    ...
    bert_wp_tokenizer = BertWordPieceTokenizer(vocab_path, lowercase=True)
    bert_wp_tokenizer.enable_padding(direction="right", pad_type_id=1)
    bert_wp_tokenizer.enable_truncation(max_tokens)
    return BertSquadTokenizer(bert_wp_tokenizer, device=device)


def get_standard_tokenizer(
    vocab, max_context_tokens, unk_token="[UNK]", pad_token="[PAD]", device="cpu"
):
    standard_question_tokenizer = Tokenizer(WordLevel(vocab, unk_token=unk_token))
    standard_question_tokenizer.normalizer = Sequence(
        [StripAccents(), Lowercase(), Strip()]
    )
    standard_question_tokenizer.pre_tokenizer = PreSequence(
    ...
    standard_context_tokenizer = Tokenizer(WordLevel(vocab, unk_token=unk_token))
    standard_context_tokenizer.normalizer = Sequence(
        [StripAccents(), Lowercase(), Strip()]
    )
    standard_context_tokenizer.pre_tokenizer = PreSequence(
        [Whitespace(), Punctuation()]
    )
