...
def parse_args():
    ...
    parser.add_argument(
        "--wrong-file", "-w", metavar="wrong.json", help="Write wrong answers to file.",
    )
    parser.add_argument(
        "--na-prob-file",
        "-n",
...
def get_raw_scores(dataset, preds):
    ...
    for article in dataset:
        for p in article["paragraphs"]:
            for qa in p["qas"]:
                ...
                if not bool(exact_scores[qid]):
                    normalized_answers = [normalize_answer(g) for g in gold_answers]
                    wrong[qid] = {
                        "context": p["context"],
    ...
    return exact_scores, f1_scores, wrong
...
def main():
    ...
    exact_raw, f1_raw, wrong = get_raw_scores(dataset, preds)
    if OPTS.wrong_file:
        with open(OPTS.wrong_file, "w") as f:
            json.dump(wrong, f)
